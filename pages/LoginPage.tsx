...
const LoginPage: React.FC = () => {
  ...
  const { login, systemSettings } = useData();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const user = await login(registrationNumber, password, rememberMe, Role.STUDENT);
      if (user) {
        navigate('/student');
      } else {
      ...
      <div className="max-w-md w-full space-y-8 p-10 bg-white dark:bg-gray-800 rounded-xl shadow-lg">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          <div className="rounded-md shadow-sm -space-y-px">
            <div>
              ...
              <input
                ...
                placeholder="Registration Number (E.g 20xx-xx-xxxxx)"
          ...
          <div className="text-center text-sm">
            {systemSettings.allowStudentRegistration && (
